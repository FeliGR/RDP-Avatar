...
// Load avatar model
export const loadAvatarModel = async (scene, modelPath = '/models/avatar.glb') => {
  try {
...
export const updateAvatarBasedOnPersonality = (avatar, personalityTraits) => {
  ...
  if (head && head.material) {
    ...
    headMaterial.diffuseColor = new BABYLON.Color3(0.9, 0.7 * intensity, 0.5 * intensity);
  }
  ...
  if (body && body.material) {
    ...
    const leftArm = avatar.getChildMeshes().find(mesh => mesh.name === "leftArm");
    const rightArm = avatar.getChildMeshes().find(mesh => mesh.name === "rightArm");
