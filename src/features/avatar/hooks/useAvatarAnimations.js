...
export const useAvatarAnimations = (scene, shadowGenerator = null) => {
  ...
  useEffect(() => {
    console.log('Animation hook effect triggered, scene:', !!scene, 'service:', !!animationServiceRef.current);
  ...
  const loadAvatarAnimations = useCallback(async (avatarUrl, options = {}) => {
    ...
    try {
      ...
      const result = await animationServiceRef.current.loadCharacter(
        avatarUrl,
        animationPaths
      );
  ...
  const _cleanupDuplicateAvatars = useCallback(() => {
    ...
    meshes.forEach(mesh => {
      ...
      const isScaledDown = mesh.scaling && (
        mesh.scaling.x < 0.01 || mesh.scaling.y < 0.01 || mesh.scaling.z < 0.01
      );
   ...
   */
  const playAnimation = useCallback(async (animationName, options = {}) => {
    if (!animationServiceRef.current || !currentCharacter) {
      return { success: false, error: 'Service or character not available' };
    }
   ...
   */
  const setMorphTarget = useCallback((morphName, value, duration = 0) => {
    if (!animationServiceRef.current || !currentCharacter) {
      return { success: false, error: 'Service or character not available' };
    }
